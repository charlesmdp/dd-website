...
def build(g):
 ...
 for i,old in enumerate(original):
  ...
  content,toc,new_cover=enrich(i,content,g)
  count=len(BeautifulSoup(content,'html.parser').get_text(' ',strip=True).split());mins=max(3,math.ceil(count/220))
  a={'slug':slug,'title':title,'category':category,'description':lead,'content':content,'toc':toc,'minutes':mins,'words':count,'sources':article_sources,'cover_path':new_cover or '/assets/editorial/blog--'+slug+'.webp'};articles.append(a)
 def card(a):return f'<a class="site-blog-card" data-search="{esc((a["title"]+" "+a["description"]).lower(),quote=True)}" data-category="{esc(a["category"],quote=True)}" href="/blog/{a["slug"]}"><img src="{a["cover_path"]}" alt="" width="1536" height="1024" loading="lazy" decoding="async"><div class="text"><small>{a["category"]} · {a["minutes"]} min read</small><h3>{esc(a["title"])}</h3><p>{esc(a["description"])}</p><span class="read">Read the guide →</span></div></a>'
 for a in articles:
  path='/blog/'+a['slug'];toc='<details class="article-toc"><summary><span>In this guide</span><span class="toc-label">Jump to a section +</span></summary><nav aria-label="Article contents">'+''.join(f'<a href="#{i}">{esc(t)}</a>' for i,t in a['toc'])+'</nav></details>'
  body=f'<div class="reading-progress" aria-hidden="true"></div><header class="article-hero"><div class="site-wrap"><div class="site-breadcrumb"><a href="/">Home</a> / <a href="/blog">The BIG journal</a> / {a["category"]}</div><div class="article-heading-grid"><div><span class="site-kicker">{a["category"]}</span><h1>{esc(a["title"])}</h1><div class="article-byline"><img src="/assets/media/3aed8c41fe6de0b62874.png" width="40" height="40" alt=""><div><strong>The BIG team</strong><span>22 Sep 2026 · {a["minutes"]} min read</span></div></div></div><img class="article-cover" src="{a["cover_path"]}" width="1536" height="1024" alt="" fetchpriority="high"></div></div></header><div class="site-wrap article-content-wrap"><div class="site-editorial"><article class="site-prose">{toc}{a["content"]}<p class="article-plain"><a href="{path}.md">Read this guide as plain text {g["icon"]("arrow")}</a></p></article></div></div>'
